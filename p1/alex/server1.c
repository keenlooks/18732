...
#ifdef USE_MMAP
#include <sys/mman.h>
#define START_ADDR (void *) 0x55387A00
#endif
...
int processbuf(int buflen, char *buf, int nfd) 
{

	char reply[FILESIZE], errorbuf[200];
	struct geotag gt;
	struct {
	  int i;
	  int j;
	...
	sprintf(reply, "Got a geotag for the image: %s", gt.img);
	...
	while (1) 
	{
		...
		for (count=0;count<next;count++) 
		{
			...
			if (FD_ISSET(con[count].nfd, &readfds)) 
			{
				...
				{
					//process the request
					strncpy(proc_buf, con[count].bufcon, 1000);
					if(validatebuf(con[count].buflen, proc_buf))
					{
                                                processbuf(con[count].buflen, proc_buf, con[count].nfd);
					}
